...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchSideeffect = async () => {
      const res = await axios(
        `https://cryptic-stream-86241.herokuapp.com/user/${email}`
      );
      setMatch(res.data);
    };
    fetchSideeffect();
    ...
    <>
      ...
      {user && (
        <>
          ...
          <li className="hover:text-secondary">
            <Link className="pl-5" to="/courses">
              Courses
            </Link>
          </li>
          <li className="hover:text-secondary">
            {" "}
            <Link className="pl-5" to="/contact">
              {" "}
              Contact Us
            </Link>
          </li>
  ...
  const userProfileNameLogo = (
    <>
      ...
      <img
        src={match?.image || dummyImage}
        className="w-9 border-red-800 border-2 rounded-full mx-auto items-center h-9"
        alt=""
      />
    </>
  );
  ...
  const userProfileDes = (
    <>
      <li>
        <Link to="/viewprofile" className="pl-5">
          {" "}
          View Profile
        </Link>
      </li>
      {match.student == "enrolled" && (
        <>
          <li>
            <Link to="/submittedTask" className="pl-5">
              {" "}
              Submitted Task
            </Link>
          </li>
          <li>
            <Link to="/analytics" className="pl-5">
              Students Analytics
            </Link>
          </li>
          <li>
            <Link to="/addReview" className="pl-5">
              Students Review
            </Link>
          </li>
          <li>
            <Link to="/announcement" className="pl-5">
              Announcement
            </Link>
          </li>
        </>
      ...
      <li>
        <Link className="pl-5" to="" onClick={logout}>
          Sign Out
        </Link>
      </li>
    </>
  ...
  const userNotificationIndicator = (
    <>
      <div className="indicator">
        <span className="indicator-item  badge bg-red-600 border-0 w-5 text-[10px]">
          {newArray?.length}
        </span>
        <div className="grid place-items-center">
          <img className="w-5 inline bg-white" src={notificationIcon} alt="" />
        </div>
      </div>
    </>
  );
    ...
    <>
      {notifications.map((notification) => (
        <li
          ...
          onClick={() => setNoti(notification._id)}
          className={
            notification.read == true
              ? "line-through p-2 border  hover:bg-blue-100 cursor-pointer"
              : "p-2 border  hover:bg-blue-100 cursor-pointer"
          }
        >
          {notification.notice}{" "}
  ...
  const menuItem2 = (
    <>
      ...
      {user && match.student == "enrolled" && (
        <div className="dropdown hidden md:block">
          <label tabIndex={1} className="">
            <li className="pl-5 cursor-pointer">{userNotificationIndicator}</li>
          </label>
          <ul
            tabIndex={1}
            className="menu dropdown-content p-2 shadow bg-base-100 rounded-box w-52 mt-4 h-[380px] overflow-auto"
          >
      ...
      {user && match.student == "enrolled" && (
        <div className="collapse md:hidden text-center">
      ...
      <>
        {match?.role == "admin" && (
          <li className="hover:text-secondary text-center c-Pannel">
            {" "}
      ...
      {!user && (
        <li>
          <Link className="ml-4 sm:ml-0 pl-5" to="/login">
      ...
      {!(match.role == "admin") && (
        <>
          {!(match.student == "enrolled") && !user && (
            <li>
              <Link className="pl-5 ml-4 sm:ml-0" to="/register">
      ...
      <div className="max-w-screen-xl mx-auto">
        <div className="navbar bg-white bg-opacity-70 Nav">
          <div className="navbar-start">
            ...
            <div
              id="mySidenav"
              className={open == true ? "sidenav shadow-lg" : "sidenav1"}
            >
            ...
            <p className=" normal-case  ml-3 font-bold text-2xl text-[#383D8A]">
              {" "}
              <Link to="/">TASKLA</Link>
            </p>
          </div>
